...
/// Magic number constant for identifying Netsody protocol messages.
...
/// Long header format for Netsody protocol messages.
///
/// The `LongHeader` represents the complete header format used in Netsody network messages.
...
/// # Header Fields
///
/// * `magic_number` - Protocol identifier to distinguish Netsody messages (constant: 507,374,529)
...
/// * `network_id` - Network identifier to isolate different Netsody networks
...
///
/// ```rust,ignore
/// use netsody_p2p::message::{LongHeader, MessageType};
/// use netsody_p2p::identity::{Identity, PoW};
///
...
/// necessary information for routing messages through the Netsody network. For
...
/// Message type identifier for Netsody protocol messages.
///
/// This structure represents the type of message being transmitted in the Netsody network.
...
///
/// The Netsody protocol defines four primary message types:
///
...
///
/// ```rust
/// use netsody_p2p::message::MessageType;
///
...
/// Magic number type for identifying Netsody protocol messages in the header.
#[repr(transparent)]
#[derive(Debug, PartialEq, Clone, Copy, FromBytes, IntoBytes, KnownLayout, Immutable)]
...
/// Network identifier type for isolating different Netsody networks.
