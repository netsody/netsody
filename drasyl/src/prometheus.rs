...
lazy_static! {
    ...
    pub static ref PROMETHEUS_MESSAGES: CounterVec =
        register_counter_vec!("drasyl_messages", "Number of messages received/sent.", &[
            "type", // ack, app, hello, unite
            "peer",
            "direction", // rx, tx
    ]).unwrap();
    pub static ref PROMETHEUS_BYTES: CounterVec =
        register_counter_vec!("drasyl_bytes", "Number of bytes transmitted.", &[
            "peer",
            "direction", // rx, tx
            "path", // direct, relayed
    ]).unwrap();
